        ...
        <form action="{{ route('guarantors.update', $guarantor->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @if ($guarantor->image)
                <div class="mb-3">
                    ...
                    <img src="{{ asset($guarantor->image) }}" alt="Guarantor Image" width="120" class="img-thumbnail">
                </div>
            @endif
            <div class="mb-3">
                <label for="customer_id">Update Image</label>
                ...
                <select name="customer_id" id="customer_id" class="form-control" required>
                    @foreach ($customers as $customer)
                        <option value="{{ $customer->id }}"
                            {{ $guarantor->customer_id == $customer->id ? 'selected' : '' }}>
                            {{ $customer->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="father_name" class="form-control" value="{{ $guarantor->father_name }}"
                            required>
                        @error('father_name')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="relation" class="form-control" value="{{ $guarantor->relation }}"
                            required>
                        @error('relation')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                ...
                if (customerId && guarantorNo) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.exists) {
                                $('#guarantor_no_error').text('Guarantor number ' + guarantorNo +
                                    ' already exists for this customer.').show();
