...
<head>
    ...
    <title>SuperAdmin Panel — Installment Management</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-value { font-size: 36px; font-weight: 800; margin: 8px 0 4px; color: #f1f5f9; }
...
<div class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('superadmin.tenants.index') }}" class="nav-item">
            <i class="fas fa-store"></i> All Stores
        </a>
        <a href="{{ route('superadmin.tenants.create') }}" class="nav-item">
...
<div class="main">
    <div class="page-header">
        ...
        <p>Sab stores (tenants) ka overview — {{ now()->format('d M Y, h:i A') }}</p>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card yellow">
            ...
            <div class="stat-value">{{ $stats['inactive_tenants'] }}</div>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="fas fa-store" style="color:#818cf8;margin-right:8px"></i>All Stores / Branches</h3>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="actions">
                            <a href="{{ route('superadmin.tenants.show', $tenant) }}" class="btn btn-sm btn-info">
                            ...
                            <form method="POST" action="{{ route('superadmin.tenants.toggle-status', $tenant) }}" style="display:inline">
                                @csrf
                                <button type="submit" class="btn btn-sm {{ $tenant->status === 'active' ? 'btn-danger' : 'btn-success' }}">
                ...
                <tr>
                    <td colspan="9" style="text-align:center;padding:40px;color:#475569">
                        ...
                        Abhi koi store nahi — <a href="{{ route('superadmin.tenants.create') }}" style="color:#818cf8">Pehla store banain</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        @if($tenants->hasPages())
        <div style="padding:16px 20px;border-top:1px solid #1e3a5f">
            {{ $tenants->links() }}
        </div>
        @endif
    </div>
